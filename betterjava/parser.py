...
def parse_bjava(bjava_code: str) -> None:
    ...
    for line_num, line in enumerate(lines, start=1):
        ...
        if stripped.endswith(":"):
            ...
            logger.error(f"Line {line_num}: Unexpected indentation for inline code.")
            raise BetterJavaSyntaxError("Unexpected indentation for inline code.", line_num)

        elif indent_stack and current_indent < indent_stack[-1]:
            while indent_stack and current_indent < indent_stack[-1]:
                logger.debug(f"Line {line_num}: Closing block at indent level {indent_stack[-1]}.")
                indent_stack.pop()
            logger.debug(f"Line {line_num}: Valid code after block closure.")

        elif current_indent == 0:
            logger.debug(f"Line {line_num}: Top-level inline code (valid).")

        else:
            logger.error(f"Line {line_num}: Misaligned or invalid code.")
            raise BetterJavaSyntaxError("Misaligned or invalid code.", line_num)
